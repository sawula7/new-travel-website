      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-12">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="text-[#6B7B7B] text-sm leading-relaxed mb-6 max-w-xs">
          ...
          {Object.entries(links).map(([title, items]) => (
            <div key={title}>
              ...
              <ul className="space-y-3">
                {items.map((item) => (
                  <li key={item}>
                    <Link href="#" className="text-[#6B7B7B] hover:text-[#2EC98A] text-sm transition-colors duration-300">
                      {item}
                    </Link>
                  </li>
                ))}
        ...
        <div className="border-t border-[#E2F5F0] pt-8 grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {[
            ...
            <div key={c.label} className="flex items-center gap-3 text-[#6B7B7B] text-sm">
              <span>{c.icon}</span>
              <span>{c.label}</span>
            </div>
          ))}
        ...
        <div className="border-t border-[#E2F5F0] pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6">
            {["Privacy Policy", "Terms of Service", "Cookie Policy"].map((item) => (
              <Link key={item} href="#" className="text-[#6B7B7B] hover:text-[#2EC98A] text-xs transition-colors duration-300">
                {item}
              </Link>
            ))}
          </div>
